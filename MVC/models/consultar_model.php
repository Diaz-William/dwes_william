<?php
    function consultarAlquileres($fechadesde, $fechahasta) {
        try {
            ...
            $id = $id[1];
            $conn = conectar();
            $stmt = $conn->prepare("SELECT MATRICULA, MARCA, MODELO, FECHA_ALQUILER, FECHA_DEVOLUCION, PRECIOTOTAL FROM RALQUILERES RA, RVEHICULOS RV WHERE RA.MATRICULA = RV.MATRICULA AND IDCLIENTE = :IDCLIENTE AND FECHA_ALQUILER BETWEEN :FECHADESDE AND :FECHAHASTA AND FECHA_DEVOLUCION IS NULL AND PRECIOTOTAL IS NULL AND FECHAHORAPAGO IS NULL ORDER BY FECHA_ALQUILER");
